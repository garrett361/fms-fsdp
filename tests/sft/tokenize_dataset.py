...
if __name__ == "__main__":
    ...
    # Comma separate seq_lens to filter on
    ...
    if data_path.is_file():
        assert data_path.suffix in [".json", ".jsonl"], f"{data_path.suffix=}"
    ...
    if args.save_path:
        ...
        if args.seq_lens:
            for seqlen in sorted(
                [int(s) for s in args.seq_lens.split(",")], reverse=True
            ):
